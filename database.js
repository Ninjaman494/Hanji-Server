...
const SEARCH_ENG = "SELECT word FROM korean_english WHERE word LIKE '%다' and (UPPER(def) LIKE UPPER($1) OR UPPER(def) LIKE UPPER($2))";
...
this.searchEng = function (term,callback) {
    let client = new pg.Client(conString);
    client.connect();
    client.query(SEARCH_ENG,['% '+term,term],(err, res) => {
        let results = [];
        console.log(err ? err.stack : res.rows);
        res.rows.forEach(function(element){
