...
describe('Mutations', () => {
    test('Create an entry suggestion', async () => {
        ...
        const res = await mutate({ mutation: CREATE_SUGGESTION, variables: { suggestion} });
        expect(res.data).not.toBe(null);
        expect(res.data.createEntrySuggestion).toBeDefined();

        expect(res.data.createEntrySuggestion.success).toBeTruthy();
        expect(res.data.createEntrySuggestion.message).toEqual("Entry suggestion successfully created");
